...
export function parseAuthError(error: AuthError | Error | null): {
  ...
  if (
    ...
    return {
      ...
      suggestion:
        'Try signing in instead, or use "Forgot Password" if you don\'t remember your password.',
      action: "signin",
    };
  }
 ...
 */
export async function checkEmailProvider(email: string, supabase: any): Promise<ProviderInfo> {
  try {
