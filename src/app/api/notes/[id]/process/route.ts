...
export async function POST(_req: Request, { params }: { params: Promise<{ id: string }> }) {
    ...
    return NextResponse.json(
      {
        ...
        message:
          "AI-powered analysis is a Premium feature. Upgrade to unlock smart connections and summaries!",
  ...
  try {
    ...
    if (!baseText || baseText.trim().length < 3) {
      ...
      return NextResponse.json({ error: "Not enough content to process" }, { status: 400 });
    }
    ...
    const shouldOverwriteTitle = note.type !== "url" || !note.title || note.title.trim().length < 3;
    ...
    for (const other of allOtherNotes) {
      ...
      if (embedding.length > 0 && otherEmb && otherEmb.length > 0) {
        ...
        if (srcTopicsArray.length > 0 && otherTopicsArray.length > 0) {
          ...
          const srcWords = new Set(srcTopicsArray.flatMap((t) => t.toLowerCase().split(/\s+/)));
          const otherWords = new Set(otherTopicsArray.flatMap((t) => t.toLowerCase().split(/\s+/)));
          ...
          for (const src of srcTopicsArray) {
            for (const other of otherTopicsArray) {
              if (
                src.toLowerCase().includes(other.toLowerCase()) ||
      ...
      if (sim > 0.3) {
        // Lower threshold for topic similarity
