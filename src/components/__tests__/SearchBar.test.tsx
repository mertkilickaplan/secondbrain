...
describe("SearchBar", () => {
  ...
  it("does not render when closed", () => {
    render(<SearchBar isOpen={false} onClose={mockOnClose} onSelectNode={mockOnSelectNode} />);
    expect(screen.queryByPlaceholderText(/search/i)).not.toBeInTheDocument();
  });

  it("renders search input when open", () => {
    render(<SearchBar isOpen={true} onClose={mockOnClose} onSelectNode={mockOnSelectNode} />);
  ...
  it("displays search results", async () => {
    (global.fetch as jest.Mock).mockResolvedValue({
      ok: true,
      json: async () => ({
        results: [{ id: "1", title: "Test Note", summary: "A test note", status: "ready" }],
      }),
    });
